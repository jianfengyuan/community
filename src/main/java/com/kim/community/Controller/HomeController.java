...
public class HomeController implements CommunityConstant {
    ...
    public String getIndexPage(Model model, Page page) {
        // 在spring MVC中, 方法調用之前 會自動實例化Model和Page, 並將Page注入model
        // 所以在 thymeleaf 中可以直接訪問Page對象中數據
        ...
        if (list != null) {
            for (DiscussPost discussPost : list) {
                ...
                Long likeCount = likeService.findEntityCount(ENTITY_TYPE_POST, discussPost.getId());
                map.put("user", user);
